...
void usage() { 
  cerr << "Usage:" << endl
       ...
       << "  -d: print register values in decimal" << endl
       << "  -r: simulate the initial reset cycle (non IO tests)" << endl
       << endl;
  exit(0);
}
...
{
  bool verbose = false, insncount = false, test_reset = false;
  ...
  if (test_reset) {
    cout << "Extra cycle for initial reset of processor. All regs are 0." << endl;
    count++;
  }
  ...
  while(!quit)
  {
    ...
    {
      show_regs();
      cout << hex << setw((ADDR_BITS+3)/4) << setfill('0') << pc << ' ' << count << ' ' << setw((ADDR_BITS+3)/4) << setfill('0') << insn.bits << dec;
    ...
    switch((char)insn.itype.opcode) // the cast is just to suppress warnings about the unsupported opcodes
    {
      ...
      */
      ...
      case OPCODE_OUTPUT:
        ...
        break;
      case OPCODE_INPUT:
        reg[insn.rtype.rd] = inputs_fail ? 128 : getchar();
        break;
      default:
